...
在构建代码并部署到生产环境之前，我们**需要一个本地环境，用于运行我们开发的代码**。这个环境相当于**提供了一个简单的服务器**，用于访问 webpack 构建好的**静态文件**，我们日常开发时可以**使用它来调试前端代码**。

> 静态文件应该是指一开始就存在于服务器上的文件。而动态文件是由服务器根据请求的不同或者其他情况下所生成的文件。
...
webpack-dev-server 本质上也是调用 webpack，4.x 版本的也要指定 mode，其实 webpack-dev-server 应该直接把 development 作为默认值，有兴趣的同学可以查看这个 issue：[Default mode to development?](https://link.juejin.im/?target=https%3A%2F%2Fgithub.com%2Fwebpack%2Fwebpack-dev-server%2Fissues%2F1327)。
...
![webpack-dev-server èµæºåè¡¨](assets/1623bff0f463e073)
...
[webpack-dev-middleware](https://link.juejin.im/?target=https%3A%2F%2Fgithub.com%2Fwebpack%2Fwebpack-dev-middleware) 就是在 Express 中提供 webpack-dev-server 静态服务能力的一个中间件，我们可以很轻松地将其集成到现有的 Express 代码中去，就像添加一个 Express 中间件那么简单。
