...
`noParse` 是整个 Webpack 完全忽略的目录，甚至连 `require`、`import` 都不会处理，所以它应该是完全独立的模块（比如某些打包好的框架类库）。`exclude` 只对 loader 生效，或是 loader 忽略该目录，最常见的用法是忽略 node*modules。但是当我们只想对 node*modules 中的某几个目录使用 loader 时，也可以通过上面例子中的正则匹配来实现。
