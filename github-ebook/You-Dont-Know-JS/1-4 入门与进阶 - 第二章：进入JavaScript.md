...
在前一章中，我介绍了编程的基本构建块儿，比如变量，循环，条件，和函数。当然，所有被展示的代码都是JavaScript。但是在这一章中，为了作为一个JS开发者入门和进阶，我们想要特别集中于那些你需要知道的关于JavaScript 的事情。
...
正如我们在第一章中宣称的，JavaScript 拥有带类型的值，没有带类型的变量。下面是可用的**内建类型**：
...
[![img](assets/fig4.png)](https://github.com/getify/You-Dont-Know-JS/blob/1ed-zh-CN/up%20%26%20going/fig4.png)
...
[![img](assets/fig5.png)](https://github.com/getify/You-Dont-Know-JS/blob/1ed-zh-CN/up%20%26%20going/fig5.png)
...
我们刚刚讨论的**内建类型**和**子类型**拥有十分强大和有用的行为，它们作为属性和方法暴露出来。
...
我们不会详细地讲解强制转换在`==`比较中是如何工作的。它的大部分都是相当合理的，但是有一些重要的极端用例要小心。你可以阅读ES5语言规范的11.9.3部分（[http://www.ecma-international.org/ecma-262/5.1/）来了解确切的规则，而且与围绕这种机制的所有负面炒作比起来，你会对这它是多么的直白而感到吃惊。](http://www.ecma-international.org/ecma-262/5.1/%EF%BC%89%E6%9D%A5%E4%BA%86%E8%A7%A3%E7%A1%AE%E5%88%87%E7%9A%84%E8%A7%84%E5%88%99%EF%BC%8C%E8%80%8C%E4%B8%94%E4%B8%8E%E5%9B%B4%E7%BB%95%E8%BF%99%E7%A7%8D%E6%9C%BA%E5%88%B6%E7%9A%84%E6%89%80%E6%9C%89%E8%B4%9F%E9%9D%A2%E7%82%92%E4%BD%9C%E6%AF%94%E8%B5%B7%E6%9D%A5%EF%BC%8C%E4%BD%A0%E4%BC%9A%E5%AF%B9%E8%BF%99%E5%AE%83%E6%98%AF%E5%A4%9A%E4%B9%88%E7%9A%84%E7%9B%B4%E7%99%BD%E8%80%8C%E6%84%9F%E5%88%B0%E5%90%83%E6%83%8A%E3%80%82)
...
等一下，这三个比较怎么可能都是`false`？因为在`<`和`>`的比较中，值`b`被强制转换为了“非法的数字值”，而且**语言规范说`NaN`既不大于其他值，也不小于其他值。**
...
**警告：** 在一个作用域中依靠变量提升来在`var`声明出现之前使用一个变量是不常见的，也不是个好主意；它可能相当使人困惑。而**使用被提升的函数声明要常见得多，也更为人所接受，就像我们在`foo()`正式声明之前就调用它一样。**
...
使用strict模式的一个关键不同（改善！）是，它**不允许因为省略了`var`而进行隐含的自动全局变量声明**：
...
如果你在代码中打开 strict 模式，并且得到错误，或者代码开始变得有 bug，这可能会诱使你避免使用 strict 模式。但是纵容这种直觉不是一个好主意。如果strict模式在你的程序中导致了问题，那么这标志着在你的代码中几乎可以肯定有应该修改的东西。
...
因为IIFE只是一个函数，而函数可以创建变量 *作用域*，以这样的风格使用一个 IIFE 经常被用于定义变量，而这些变量将不会影响围绕在IIFE外面的代码：
...
1. 当我们调用`makeAdder(1)`时，我们得到一个指向它内部的`add(..)`的**引用**，它记住了`x`是`1`。我们称这个函数引用为`plusOne(..)`。
...
但是相信我，一旦你理解了它，它就是编程中最强大最有用的技术之一。让你的大脑在闭包中煎熬一会儿是绝对值得的。在下一节中，我们将进一步实践闭包。
...
当你引用一个对象上的属性时，如果这个属性不存在，JavaScript将会自动地使用这个对象的内部原型引用来寻找另外一个对象，在它上面查询你想要的属性。你可以认为它几乎是在属性缺失时的**备用对象**。
...
[![img](assets/fig6.png)](https://github.com/getify/You-Dont-Know-JS/blob/1ed-zh-CN/up%20%26%20going/fig6.png)
...
if (!Number.isNaN) {
	Number.isNaN = function isNaN(x) {
		return x !== x; // 在 JS 的中，使用变量的形式比较时，只有 NaN 不等于它自己
	};
}
```
...
**注意：** 我们在这里做的检查利用了`NaN`值的怪异之处，即**它们是整个语言中唯一与自己不相等的值**。所以`NaN`是唯一可能使`x !== x`为`true`的值。

并不是所有的新特性都可以完全填补。有时一种特性的大部分行为可以被填补，但是仍然存在一些小的偏差。在实现你自己的填补时你应当非常非常小心，来**确保你尽可能严格地遵循语言规范**。
