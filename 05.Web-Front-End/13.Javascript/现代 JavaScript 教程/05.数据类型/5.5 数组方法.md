...
这很正常，因为 `delete obj.key` 是通过 `key` 来移除对应的值。对于对象来说是可以的。但是对于数组来说，我们通常希望剩下的元素**能够移动**并**占据被释放的位置**。我们希望得到一个更短的数组。

所以应该使用特殊的方法。

[arr.splice(str)](https://developer.mozilla.org/zh/docs/Web/JavaScript/Reference/Global_Objects/Array/splice) 方法可以说是**处理数组的瑞士军刀**。它可以做所有事情：添加，删除和插入元素。
...
它会返回一个新数组，将所有从索引 `start` 到 `end`（不包括 `end`）的数组项**复制到一个新的数组**。`start` 和 `end` 都可以是负数，在这种情况下，从末尾计算索引。

它和字符串的 `str.slice` 方法有点像，就是把子字符串替换成了子数组。
...
我们也可以**不带参数地调用它**：`arr.slice()` 会创建一个 `arr` 的副本。其通常用于获取副本，以进行不影响原始数组的进一步转换。
...
结果是一个包含来自于 `arr`，然后是 `arg1`，`arg2` 的新数组的元素。
...
> 因此可以发现数组是带有某些特殊内置属性的对象，只是由于带有这些内置属性可以存在一些特殊的行为或是改变行为。
...
> 相较于 `arr.indexOf(item, from)` 方法而言，虽然返回的都是 `index` ，但 `findIndex` 方法传入的是回调并且可以在函数中获取到 `item` `index` `arr` ，对于查找对象数组而言是非常有利的。
...
> 这里的「转换」并没有指明是将什么转换为数组还是数组被转换为其他
...
调用带有空字符串参数的 `split(s)`，会将字符串拆分为字母数组：
...
[arr.reduce](https://developer.mozilla.org/zh/docs/Web/JavaScript/Reference/Global_Objects/Array/reduce) 方法和 [arr.reduceRight](https://developer.mozilla.org/zh/docs/Web/JavaScript/Reference/Global_Objects/Array/reduceRight) 方法和上面的种类差不多，但稍微复杂一点。它们用于根据数组计算返回单个值。
...
应用函数时，上一个函数调用的结果将作为第一个参数**传递给下一个函数**。

> 这里说明，像 `map` `filter` `find` `reduce` 等传入回调来遍历数组的方法，其过程是对每一个数组元素执行一次回调。
...
![image-20200301234058891](assets/image-20200301234058891.png)
...
> ```
>
> - `(*)` 行，由于上一行移除了 `arr` 中的一个元素，则相应的也要减少 `i` 才能使遍历不跳过其中的任何元素。
...
[打开带有测试的沙箱。](https://plnkr.co/edit/5KBkJX6foC1LCRrTjIDh?p=preview)
...
> ```
>
> - 一开始有个问题，当满足使用 `addMethod` 方法添加的运算方式时会返回 `undefined` ，原因是我一开始是在 `forEach` 内部进行 `return` 的，而不是像现在这样将 `method` 的结果返回给 `res` 
...
> > 相比于我的方案，教程的方案有如下优点：
> >
> > - 使用键值对的方式将运算符和运算方法存入对象 `this.methods` 的中，便可直接通过访问对象属性的方式来调用运算方法。而我是将运算符和运算方法作为一个对象的两个属性存入一个数组中，再通过遍历判断的方式来执行对应的运算方法，就显得很冗余。
> > - 做了错误验证，验证了输入字符串的值，以避免发生错误。
> > - 当然，由于教程还没有讲解 `class` 和解构，所以看起来像是很老旧的代码。
...
>
> 为什么它不起作用？一般来说，`sort` 是一个“黑匣子”：我们将一个数组和一个比较函数放入其中，并期望其对数组进行排序。但是由于比较的完全随机性，这个黑匣子疯了，它**发疯地确切程度**取决于引擎中的具体实现方法。
>
...
> ```
>
> - `(*)` 行是使用数组解构赋值的方式对 `array[i]` 和 `array[j]` 进行赋值
>
> 让我们以相同的方式测试一下：
>
> ```javascript
...
> 
>   for (let str of arr) {
>     if (!result.includes(str)) { // 这一步也会进行遍历
>       result.push(str);
>     }
>   }
> 
...
>
> [使用沙箱的测试功能打开解决方案。](https://plnkr.co/edit/95r2YFm0Uql8KEJU4aJZ?p=preview)
...
> 要先去考虑为什么处理服务器数据时有用，是因为可以将数组转换为对象，这样就能通过键来快速获取值了吗？
>
> 如果是这样，确实可以考虑自己封装一个这样的函数。

在这个任务里我们假设 `id` 是唯一的。没有两个具有相同 `id` 的数组项。

请在解决方案中使用数组的 `.reduce` 方法。

[打开带有测试的沙箱。](https://plnkr.co/edit/NzbRgQkthTQexAMXh3y1?p=preview)
