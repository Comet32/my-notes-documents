...
首先，我们在源码中有一个全局的函数 `globalLib` ，在声明文件中，我们也提供了一个全局的声明 `declare function globalLib`，然后他的参数是一个 `options` ，`options` 是一个对象，这里我们使用一个接口来约束这个对象的结构，这个接口我们放在了一个命名空间 `globalLib` ，接口是一个可索引类型的接口，它可以接受任意的字符串属性，然后返回值也是 `any` 。
...
接下来看两种插件——模块插件与全局插件，它的意义是，允许我们想给一个类库添加一些自定义的方法。比如我们想给 `moment` 类库增加一些自定义的方法。
