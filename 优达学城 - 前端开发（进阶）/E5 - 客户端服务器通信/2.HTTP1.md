...
要成为优秀的前端工程师并创建网络应用仅仅使用 HTTP 并不足够，你需要深入了解为何使用某些技巧，为何其他技巧不可行或不推荐使用。

在这节课，我们将先了解下第一版 HTTP，我们将研究 GET 和 POST 方法，以及其他你可能会熟悉的使用方法。接着，我们将了解各种报头以及如何设计适用于 web 的 API。

学完这节课后，你将能够调试网络上的 HTTP 请求响应并且将了解什么是 RESTful API。
...
- 客户端请求查看 logo.png 上次更新的。
  - 客户端想要查看 logo.png 上次修改的时间，我们知道 last modified 报头具有该信息，因为客户端对文件实际内容不感兴趣，因此这里适合使用 `HEAD` 请求。

- 客户端发送了关于表单提交的数据。
  - **表单只能使用两种方法**，GET 和 POST，因为我们是要发送数据，因此需要使用 `POST` 方法。（但这个论据其实并不充分，因为 GET 也可以发送数据。）
- 客户端请求主 CSS 文件。
...
可以看出，有多种多样发送请求的方式。要使服务器正确地做出响应，务必为相应的任务使用正确的动词。
...
> 响应标题
>
> 标头包含有关请求或响应的其他数据。这些是一些重要的：
>
> `Content-Length`是**必须包含在每个响应中的标头**，并告诉浏览器响应中文档的大小。这样浏览器就知道在标题部分之后可以接收多少字节，并且**在下载文件时可以显示有意义的进度条**。
>
> `Content-Type`也是一个非可选标题，**它告诉您文档的类型。这样浏览器就知道要启动哪个解析引擎。如果是图像/ jpeg，则显示图像。这是text / html？让我们解析它并发出必要的额外HTTP请求。**等等。
>
...
>
> `Cache-Control`正是它听起来的样子。**它允许服务器控制客户端缓存收到的响应的方式和时长**。 `Cache-Control`是一个复杂的野兽，具有许多内置功能。 99％的情况下，您只需要“cacheability（可缓存性）”和“max-age（最大年龄）”。
>
> `If-Modified-Since`允许服务器跳过发送文档的实际内容（如果自该标题中提供的日期以来尚未更改）。 ETags 有类似的东西吗？就在这里！标题称为 If-None-Match 并且完全相同。**如果文档的 ETag 仍然与 If-None-Match 标头中发送的 ETag 匹配，则服务器将不发送实际文档**。 If-None-Match 和 If-Modified-Since 都可以出现在同一个请求中，但 ETag 优先于 If-Modified-Since，因为它被认为更准确。
>
...
这是我连接服务器时用到的 Netcat 命令，运行它没有产生任何错误，因此我连接成功了。（可是我出错了）
...
就好像我们和发送的字节之间几乎没有软件，但其中依然发生了很多我们无法直接看到的操作，实际上这对整个请求响应流程的速度有很大的影响。
...
**互联网协议使我们能够与互联网上的其他机器通信**，而 **TCP 使我们能够在这两台机器之间建立多个独立的数据流**，**这些数据流通过端口号加以区分**。TCP 协议还确保所有文件包都不会丢失，并且按照正确的顺序抵达，所有这些都需要耗费时间和资源成本的防范措施：
...
对于 HTTP 来说，连接就想一个队列，当第一个请求正在被处理时，所有其他请求都需要等待：
...
如果客户端将 connection 报头设为 keep-alive，服务在成功发出响应后将不会关闭连接。而是允许客户端重复利用已经建立的连接来进行其他请求：
...
因此 JavaScript 和 CSS 文件，通常都捆绑到一起，图片也会被加工为精灵图（sprites），仅用一个请求就能获得捆绑包：
