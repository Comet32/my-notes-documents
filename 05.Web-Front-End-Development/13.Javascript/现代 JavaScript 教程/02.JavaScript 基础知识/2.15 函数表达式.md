...
在 JavaScript 中，函数不是“神奇的语言结构”，而是**一种特殊的值**。
...
- **函数表达式是在语句内部的**：`let sayHi = ...;`，**作为一个值。它不是代码块而是一个赋值语句**。不管值是什么，都建议在语句末尾添加分号 `;`。所以这里的分号与函数表达式本身没有任何关系，它**只是用于终止语句**。
