...
因为服务器端也加载了 Home 组件，由于 Home 组件使用了 connect，那么也需要将服务器端运行的 React 代码中引入 Redux 来管理数据，否则会报错，在 .src/server/utils.js 中：
...
需要注意的是，render() 现在是返回页面内容，并且我们需要向 render 传入 store 和 routes，render() 又回到的之前只用户渲染页面的功能。
